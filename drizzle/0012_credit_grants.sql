...
CREATE FUNCTION assert_grant_entry_attributed() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'grant-enabled account entry requires grant attribution';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
