...
describe('DrizzleLedgerRepository', () => {
  ...
  it('listTransactions filters by ledgerId within tenant scope and keeps pagination cursor contract', async () => {
    ...
    const firstPage = await repository.listTransactions({
      tenantId,
      ledgerId: ledgerMain,
      limit: 1,
    });
  ...
  it('reverseTransaction is idempotent and links reversal to original without mutating original', async () => {
    ...
    expect(original?.reversalOfTransactionId).toBeNull();
    expect(reversal?.reversalOfTransactionId).toBe(created.transactionId);
  });
