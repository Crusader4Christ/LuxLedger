...
import { type DrizzleRepositoryContext, generateUuidV7 } from '../repository-context';
...
export class DrizzleReconciliationRepository implements ReconciliationApplicationRepository {
  ...
  public async ingest(input: IngestReconRecordsInput): Promise<ReconUpload> {
    try {
      ...
      this.context.handleDatabaseError(error, 'ingest reconciliation external records');
    }
  }
  ...
  public async createRule(input: CreateReconRuleInput) {
    try {
      ...
      this.context.handleDatabaseError(error, 'create reconciliation matching rule');
    }
  }
  ...
  public async listRules(tenantId: string) {
    try {
      ...
      this.context.handleDatabaseError(error, 'list reconciliation matching rules');
    }
  }
  ...
  public async getRule(tenantId: string, ruleId: string) {
    try {
      ...
      this.context.handleDatabaseError(error, 'get reconciliation matching rule');
    }
  }
  ...
  public async run(input: RunReconInput): Promise<ReconRun> {
    try {
      ...
      this.context.handleDatabaseError(error, 'run reconciliation');
    }
  }
  ...
  public async getRun(tenantId: string, runId: string): Promise<ReconRun | null> {
    try {
      ...
      this.context.handleDatabaseError(error, 'get reconciliation run');
    }
  }
