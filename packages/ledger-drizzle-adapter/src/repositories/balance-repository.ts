...
export class DrizzleBalanceRepository implements BalanceApplicationRepository {
  ...
  public async getTrialBalance(query: LedgerTrialBalanceQuery): Promise<TrialBalance> {
    try {
      return await this.context.withTenantTransaction(query.tenantId, async (tx) => {
        ...
        return { ledgerId: query.ledgerId, accounts, totalDebitsMinor, totalCreditsMinor };
      });
    } catch (error) {
      this.context.handleDatabaseError(error, 'get trial balance');
    }
  }
  ...
  public async getAt(query: BalanceAtQuery): Promise<HistoricalBalance> {
    try {
      ...
      this.context.handleDatabaseError(error, 'get historical balance');
    }
  }
  ...
  public async listHistory(
    ...
    try {
      ...
      this.context.handleDatabaseError(error, 'get balance history');
    }
  }
