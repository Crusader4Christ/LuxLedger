...
import type { DrizzleRepositoryContext, RepositoryLogger } from '../repository-context';
import * as schema from '../schema';
import { loadEntriesByTransactionIds } from './entry-loader';
import { DrizzlePostingSupport } from './posting-support';
...
export class DrizzleTransactionStore {
  public constructor(
    private readonly context: DrizzleRepositoryContext,
    private readonly support = new DrizzlePostingSupport(),
  ) {}
  ...
  private get logger(): RepositoryLogger {
  ...
  private withTenantContext<T>(
    ...
    return this.context.withTenantTransaction(tenantId, operation);
  }

  private handleDatabaseError(error: unknown, operation: string): never {
    return this.context.handleDatabaseError(error, operation);
  }
  ...
  public async correctTransaction(
    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        this.support.validateEntries(input.entries, original.currency);
        const originalEntries = await this.loadEntriesByTransactionIds(tx, input.tenantId, [
          input.transactionId,
        ]);
  ...
  private async createOrResolvePostedTransaction(
    ...
    await this.support.validateTransaction(tx, input);
  ...
  private async applyPostedTransaction(
    ...
    for (const entry of entriesForBalanceUpdate) {
      ...
      const [previousSnapshot] = await tx
        ...
        .limit(1);
      await this.support.insertBalanceSnapshot(tx, {
        tenantId: input.tenantId,
